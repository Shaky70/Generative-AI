...
def filter_dataset(dataset_name):
    ...
    #df['target'] = np.array(pd.Series(data.target))
    ...
    tnames= []
    tnames  = np.array(data.target_names)

    return X,y, df, tnames

X, y, df, tnames = filter_dataset(dataset_name)


st.write("\n")
st.write("Shape of the dataset is :",X.shape)
st.write("Number of targets :",len(np.unique(y)))
st.write("Targets names are :", tnames)
